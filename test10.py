...
# 動画を初期化
cap, prvs = initialize_video_capture("assets/echo_data/2_2cm.mp4")
...
while True:
    ...
    # フレームごとに上位90%の移動量を計算
    top_90_percent_movement = calculate_top_90_percent_movement(mag_full)
    total_top_90_percent_movement += top_90_percent_movement  # 上位90%の移動量の合計を更新

    frame_count += 1

    # フレームごとの上位90%の移動量を出力
    print(f"フレーム{frame_count}: 上位90%の移動量: {top_90_percent_movement:.2f}")
...
# 最終的な上位90%の移動量の合計を出力
print(f"ROIの上位90%の移動量の合計: {total_top_90_percent_movement:.2f}")
