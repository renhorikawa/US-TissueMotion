...
def process_video_with_echo_flow_95(video_path):
    ...
    # 前処理：グレースケール変換とぼかし処理（平均値フィルタ + ヒストグラム均等化）
    old_gray = cv2.cvtColor(old_frame, cv2.COLOR_BGR2GRAY)

    # 平均値フィルタによるぼかし処理
    old_gray = cv2.blur(old_gray, (5, 5))  # (5, 5) はカーネルサイズ
    ...
    while True:
        ...
        # 平均値フィルタによるぼかし処理
        frame_gray = cv2.blur(frame_gray, (5, 5))  # (5, 5) はカーネルサイズ
...
# 動画のパスを指定
video_path = 'assets/echo_data/2_3cm.mp4'
