...
def plot_histograms(mag, frame, roi, output_file='histograms.png'):
    ...
    axs[0].hist(valid_magnitudes, bins=50, color='blue', alpha=0.7, label='Movement Magnitude')
    axs[0].axvline(percentile_95, color='red', linestyle='dashed', label=f'95th Percentile: {percentile_95:.2f}')
    axs[0].set_title("Histogram of Movement Magnitudes")
    axs[0].set_xlabel("Movement Magnitude")
    axs[0].set_ylabel("Frequency")
    axs[0].legend()
    axs[0].grid(True)

    # 2つ目のグラフ：ROI内の輝度値のヒストグラム
    axs[1].plot(roi_hist, color='green', label='ROI Luminance Histogram', linewidth=2)
    axs[1].set_title("Histogram of ROI Luminance")
