...
def calculate_top_85_percent_movement(mag):
    # フレーム内で上位85%の値を計算
    valid_magnitudes = mag[mag > 1]  # 動きがあるピクセルのみ
    if valid_magnitudes.size > 0:
        top_85_percent = np.percentile(valid_magnitudes, 85)  # 上位85%の値を計算
        return top_85_percent
    else:
        return 0  # 動きがない場合は0
...
# 合計の上位85%の移動量を保存する変数
total_top_85_percent_movement = 0.0
...
while True:
    ...
    # フレームごとに上位85%の移動量を計算
    top_85_percent_movement = calculate_top_85_percent_movement(mag_full)
    total_top_85_percent_movement += top_85_percent_movement  # 上位85%の移動量の合計を更新

    frame_count += 1

    # フレームごとの上位85%の移動量を出力
    print(f"フレーム{frame_count}: 上位85%の移動量: {top_85_percent_movement:.2f}")
...
# 最終的な上位85%の移動量の合計を出力
print(f"ROIの上位85%の移動量の合計: {total_top_85_percent_movement:.2f}")
